...
class Widget_Newsmag_Flickr extends WP_Widget {
	public function __construct() {
		parent::__construct(
			'newsmag_flickr_widget',
			__( 'Newsmag - Flickr', 'newsmag' ),
			array(
				'description' => __( 'Display latest photos from flickr.', 'newsmag' ),
				'classname'   => 'newsmag-flickr'
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		<div class="flickr-widget">
			...
			<?php foreach ( (array) $data as $item ): ?>
				...
				<ul class="flickr_badge_images">
					<li class="flickr_badge_image">
						<a href="<?php echo esc_url( $item['link'] ); ?>">
							<img src="<?php echo esc_url( $item['media'] ); ?>"
	...
	public function get_flickr( $instance ) {
		...
		$data = $this->parse_script(
			'http://api.flickr.com/services/feeds/photos_public.gne?format=json&id=' . urlencode( $user_id ) . '&nojsoncallback=1&',
			$show_num
		);
	...
	public function form( $instance ) {
		...
		<p>
			...
			echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text"
			       value="<?php echo esc_attr( $title ); ?>"/>
		</p>

		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'user_id' ) ); ?>"><?php _e( 'Flickr ID (<a href="http://www.idgettr.com">Get Your ID</a>):', 'newsmag' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'user_id' ) ); ?>" name="<?php
			echo esc_attr( $this->get_field_name( 'user_id' ) ); ?>" type="text"
			       value="<?php echo esc_attr( $user_id ); ?>"/>
		</p>

		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'show_num' ) ); ?>"><?php _e( 'Number of Photos:', 'newsmag' ); ?></label>
			<input class="width100" id="<?php echo esc_attr( $this->get_field_id( 'show_num' ) ); ?>" name="<?php
			echo esc_attr( $this->get_field_name( 'show_num' ) ); ?>" type="text"
			       value="<?php echo esc_attr( $show_num ); ?>"/>
		</p>
